...
source: "https://ontologist.substack.com/p/context-graphs-a-series-of-unfortunate?utm_source=profile&utm_medium=reader2"
date: "Feb 22"
tags: [article]
---
...
![](https://substackcdn.com/image/fetch/$s_!-s-I!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F3f0c691a-9e77-49e5-8135-f71db85cc319_2688x1536.jpeg)

](https://substackcdn.com/image/fetch/$s_!-s-I!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F3f0c691a-9e77-49e5-8135-f71db85cc319_2688x1536.jpeg)
...
This created the [SHACL file](https://github.com/kurtcagle/contextGraph/blob/main/ontology/meeting-transcript-ontology.ttl), which, for purposes of discussion, I’ll let you, gentle reader, check out on your own.

This SHACL file is then used internally to prompt the creation of an [output Turtle file](https://github.com/kurtcagle/contextGraph/blob/main/examples/meeting-transcript-rdf.ttl) that converts the transcript into RDF. A sample from this file can be seen as:
...
# More content in the above link.
```

This produces a graph that looks like the following (click for full-size):

[

![](https://substackcdn.com/image/fetch/$s_!iKnx!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F7f354d3d-8ed2-4005-8d59-d078f8acfbab_7883x8192.png)

](https://raw.githubusercontent.com/kurtcagle/contextGraph/refs/heads/main/docs/mermaid-transcript-graph.png)

Once the TTL file is generated you can use it to provide a summary of what happened at the meeting, using the prompt:

```
Given the transcript TTL file, provide a summary of the meeting.
```

## Meeting Summary: Q1 2026 Product Strategy Review (Claude)
...
1.  **By Jan 15:** Schedule February checkpoint meeting (Jennifer)
    
2.  **By Jan 18:** Begin analytics dashboard planning (David)
    
3.  **By Jan 25:** Complete AI infrastructure assessment (Jennifer, Michael)
    
4.  **February:** Hold checkpoint meeting on mobile redesign progress
    
5.  **April:** Launch Android mobile app redesign
    
6.  **June:** Launch iOS mobile app redesign
    

_This summary was automatically generated from the structured RDF meeting transcript using the Meeting Transcript Ontology._
...
```
Given the SHACL and Turtle, provide three questions of the meeting and give answers to those questions that may be asked by an auditor.
```

This is the output for one of those questions:
...
_“The meeting approved a $250,000 budget allocation. Can you provide a complete audit trail showing: (a) who proposed the allocation, (b) what discussion occurred, (c) how the final amounts were justified, and (d) what accountability measures are in place?”_
...
#### (b) Discussion Record and Deliberation
...
1.  David Park ([david.park@example.com](mailto:david.park@example.com)) - Data Scientist
    
2.  Michael Chen ([michael.chen@example.com](mailto:michael.chen@example.com)) - Senior Product Manager
    
3.  Jennifer Martinez ([jennifer.martinez@example.com](mailto:jennifer.martinez@example.com)) - Engineering Lead
    
4.  Sarah Johnson ([sarah.johnson@example.com](mailto:sarah.johnson@example.com)) - VP of Product
...
user engagement, conversion rates, and feature adoption."
```
...
to mobile? The dashboard, while useful, isn't customer-facing."
```

-   **Intent Type:** amendment
...
is a strategic investment in decision-making capability."
```
...
mt:discussion-budget 
    ...
    mto:summary "Budget allocated with phased approach for analytics dashboard, 
                 ...
                 budget with contingency fund." .
```

[

![](https://substackcdn.com/image/fetch/$s_!Aokg!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F47484f04-38a0-4038-bb96-0d2bbd6cf64a_772x490.png)

](https://substackcdn.com/image/fetch/$s_!Aokg!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F47484f04-38a0-4038-bb96-0d2bbd6cf64a_772x490.png)

**Decision Rationale (from RDF):**
...
1.  **Analytics Dashboard Reduction:** Changed to Phase 1 approach ($40K) to prove value before full investment
    
2.  **Contingency Fund Addition:** Jennifer Martinez raised concerns about typical budget overruns
    
3.  **Mobile Constraints:** Lisa Anderson acknowledged tight budget but confirmed achievable with phased release (Android first, then iOS)
...
mt:individual-vote-008 mto:voter mt:participant-lisa ; mto:voteValue "for" .
```
...
-   **Assigned To:** David Park ([david.park@example.com](mailto:david.park@example.com))
...
```
mt:decision-budget-allocation 
    mto:actionItem mt:action-analytics-planning .
```
...
One final rant. There is considerable discussion in the AI community about context graphs, which are being heralded as the next major opportunity for LLMs to provide a GPS-like capability for businesses. _By itself, this is consultant-grade BS_. LLMs are not databases - they are unreliable, error-prone, processing-intensive, can’t be easily updated, and are highly ambiguous. This is true for any CNN or RNN. LLMs are effective, as I’ve demonstrated, at analysing patterns. Yes, absolutely use them in that context, but the graph itself should be purely deterministic, and that’s not going to happen with LLMs. Period.

In Media Res,

[

![](https://substackcdn.com/image/fetch/$s_!BSZH!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fba952662-3c09-489a-97eb-a046a59e6d81_2688x1536.jpeg)

](https://substackcdn.com/image/fetch/$s_!BSZH!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fba952662-3c09-489a-97eb-a046a59e6d81_2688x1536.jpeg)
