...
class GraphUpdater:
    ...
    def _validate(self, subtask_uri: str) -> list[str]:
        try:
            ...
            rule_report = RuleEngine(self.store, shapes_paths=self.shapes_paths).run()
            return [str(v.message) for v in rule_report.violations
                    if subtask_uri in str(v.subject)]
