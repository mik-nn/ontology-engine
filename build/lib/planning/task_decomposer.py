...
_PATTERNS: dict[str, list[tuple[str, str, str, str, list[int]]]] = {
    ...
    "GitTask": [
        ("AnalysisTask",  "check-status",
         "Run git status to identify changed, staged, and untracked files.",
         "git_client", []),
        ("GitTask",       "stage-changes",
         "Stage all modified and untracked files for commit.",
         "git_client", [0]),
        ("GitTask",       "commit-changes",
         "Create a git commit with a descriptive message summarising the changes.",
         "git_client", [1]),
        ("GitTask",       "push-to-remote",
         "Push the current branch to the remote repository (origin).",
         "git_client", [2]),
    ],
    "DebugTask": [
        ("AnalysisTask",  "read-logs",
         "Collect error logs, stack traces, and recent pipeline events from the graph.",
         "reasoning", []),
        ("AnalysisTask",  "trace-failure",
         "Run RuleEngine to identify constraint violations and dependency failures.",
         "reasoning", [0]),
        ("DebugTask",     "diagnose",
         "Use LLM (thinking mode) to synthesise a root-cause analysis from collected evidence.",
         "llm", [0, 1]),
    ],
    "SearchTask": [
        ("SearchTask",    "web-search",
         "Execute web search to fetch external documentation or API specs into the graph.",
         "llm", []),
        ("AnalysisTask",  "integrate-results",
         "Enrich graph with search results; rebuild context packet.",
         "reasoning", [0]),
        ("ExplainTask",   "synthesize-answer",
         "Use LLM to compose a structured answer from enriched context.",
         "llm", [0, 1]),
    ],
    "DocumentTask": [
        ("AnalysisTask",  "gather-doc-context",
         "Collect existing documentation and code descriptions via ContextBuilder.",
         "reasoning", []),
        ("DocumentTask",  "write-docs",
         "Use LLM to write or update documentation conforming to databook schema.",
         "llm", [0]),
        ("AnalysisTask",  "validate-docs",
         "Check SHACL databook shapes; verify G01–G03 gaps are resolved.",
         "reasoning", [1]),
        ("DocumentTask",  "commit-changes",
         "Commit updated documentation files.",
         "git_client", [2]),
    ],
    "ValidateTask": [
        ("AnalysisTask",  "run-shacl",
         "Execute full SHACL validation across all registered shapes.",
         "reasoning", []),
        ("AnalysisTask",  "run-rules",
         "Run RuleEngine for cycle detection and custom constraint checks.",
         "reasoning", [0]),
        ("AnalysisTask",  "produce-validation-report",
         "Compile SHACL and rule results into a structured validation report.",
         "reasoning", [0, 1]),
    ],
    "MigrateTask": [
        ("AnalysisTask",  "analyze-migration-scope",
         "Map current API/schema surface; identify changed or removed symbols.",
         "reasoning", []),
        ("DesignTask",    "design-migration-plan",
         "Use LLM (thinking) to draft a step-by-step migration plan.",
         "llm", [0]),
        ("MigrateTask",   "apply-migration",
         "Use LLM to rewrite affected modules following the migration plan.",
         "llm", [1]),
        ("AnalysisTask",  "verify-migration",
         "Run SHACL and rule engine to confirm migration is complete and valid.",
         "reasoning", [2]),
        ("MigrateTask",   "commit-changes",
         "Commit migrated files.",
         "git_client", [3]),
    ],
    "ReviewTask": [
        ("AnalysisTask",  "collect-review-context",
         "Gather changed modules, their dependants, and current SHACL report.",
         "reasoning", []),
        ("ReviewTask",    "perform-review",
         "Use LLM to assess correctness, security, performance, and style.",
         "llm", [0]),
    ],
    "IntegrateTask": [
        ("AnalysisTask",  "analyze-interfaces",
         "Map the public interfaces of the systems/modules to be integrated.",
         "reasoning", []),
        ("DesignTask",    "design-integration",
         "Use LLM to propose an integration adapter or bridge pattern.",
         "llm", [0]),
        ("IntegrateTask", "implement-integration",
         "Use LLM to write the integration layer.",
         "llm", [1]),
        ("AnalysisTask",  "verify-integration",
         "Run SHACL and cycle detection on the updated dependency graph.",
         "reasoning", [2]),
        ("IntegrateTask", "commit-changes",
         "Commit integration code.",
         "git_client", [3]),
    ],
    "ComplexTask": [
        ("AnalysisTask",  "deep-analysis",
         "Comprehensive BFS context collection at max depth; full SHACL audit.",
         "reasoning", []),
        ("DesignTask",    "reason-through-problem",
         "Use Claude with extended thinking to reason through the complex problem.",
         "llm", [0]),
        ("ImplementTask", "execute-solution",
         "Use Claude to implement the proposed solution.",
         "llm", [1]),
        ("AnalysisTask",  "verify-solution",
         "Run SHACL, cycle detection, and available tests.",
         "reasoning", [2]),
        ("ComplexTask",   "commit-changes",
